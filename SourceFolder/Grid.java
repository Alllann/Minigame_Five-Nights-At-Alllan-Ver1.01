...
public class Grid extends JComponent implements KeyListener, MouseListener
{
  ...
  public Grid(String imageFileName)
  {
    ...
    showImage(image, "background.png");
  ...
  private void init(int numRows, int numCols)
  {
    ...
    int cellSize = Math.max(Math.min(1080 / getNumRows(), 2050 / getNumCols()), 1);    
  ...
  private void showImage(BufferedImage image, String backgroundFileName)
  {
    BufferedImage backgroundImage = loadImage(backgroundFileName);
    ...
    for (int row = 0; row < getNumRows(); row++) {
      for (int col = 0; col < getNumCols(); col++) {
        ...
        if (imageFileName != null) {
          if (imageFileName.equals("user.gif") || imageFileName.equals("avoid.gif") || imageFileName.equals("get.gif")) {
            cell.setImageFileName(backgroundFileName); // Set background image
          } else {
    ...
    for (int row = 0; row < getNumRows(); row++)
    {
        for (int col = 0; col < getNumCols(); col++)
        {
            ...
            String imageFileName = cell.getImageFileName();
            if (imageFileName != null &&
                    (imageFileName.equals("user.gif") || imageFileName.equals("avoid.gif") || imageFileName.equals("get.gif")))
            {
                cell.setImageFileName(backgroundFileName); // Set background image
            }
        }
...
public void paintComponent(Graphics g) {
    ...
    // Draw the cell images and colors
    for (int row = 0; row < getNumRows(); row++) {
        for (int col = 0; col < getNumCols(); col++) {
            Location loc = new Location(row, col);
            Cell cell = cells[loc.getRow()][loc.getCol()];

            // Draw the background image for specified cells
            String imageFileName = cell.getImageFileName();
            if (imageFileName != null &&
                    (imageFileName.equals("user.gif") || imageFileName.equals("avoid.gif") || imageFileName.equals("get.gif"))) {
                URL url = getClass().getResource(imageFileName);
                if (url == null) {
                    ...
                    g.drawImage(image, x, y, getCellSize(), getCellSize(), null);
                }
            } else {
                // Draw the cell color
                Color color = cell.getColor();
                g.setColor(toJavaColor(color));
                g.fillRect(col * getCellSize(), row * getCellSize(), getCellSize(), getCellSize());
            }

            // Draw the cell outline
            if (lineColor != null) {
                g.setColor(toJavaColor(lineColor));
                g.drawRect(col * getCellSize(), row * getCellSize(), getCellSize(), getCellSize());
            }
        }
    }
}
  ...
  {
    showImage(loadImage(imageFileName), "background.png");
    setTitle(imageFileName);
  }
